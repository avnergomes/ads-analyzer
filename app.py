...
* Days.csv – daily performance by ad set.
* Days + Time.csv – daily performance broken down by hour of day.
* Days + Placement + Device.csv – performance by placement and device.
...
def build_mapping_rules(table: pd.DataFrame) -> List[MappingRule]:
    ...
    for _, rule in table.iterrows():
        ...
        try:
            ...
            st.warning(f"⚠️ Invalid regex in mapping file: {pattern!r} ({exc})")
    return rules
